...
var storageImg = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        if (req.query.uniqueId) {
            ...
            var dirPath = path.join(dirPathParent, 'img'); // 不能直接创建dirPath,因为父目录不存在会抛出异常
...
var storageFile = multer.diskStorage({
    destination: function (req, file, cb) {
        if (req.query.uniqueId) {
            var dirPathParent = path.join(__dirname, '../../public/uploads/', req.query.uniqueId);
            var dirPath = path.join(dirPathParent, 'file');
            fs.mkdir(dirPathParent, function (err) {
                if (err && err.code !== 'EEXIST') {
                    cb(err);
                } else {
...
exports.indexGet = function (req, res, next) {
    var list = [];
    var total = 0;
    var rootPath = path.join(__dirname, '../../public/uploads');
    var rootFiles;
    var parFiles;
    var files;
    var stat;
    ...
    switch (req.query.action) {
        // 获取配置
        case 'config':
            tool.getConfig(path.join(__dirname, '../../config/props/ue.json'), function (err, settings) {
                if (err) {
                    next(err);
                } else {
                    res.json(settings);
    ...
    switch (req.query.action) {
            ...
            uploadFile(req, res, function (err) {
                if (err) {
                    ...
                    res.json({
                        ...
                        url: '/uploads/' + req.query.uniqueId + '/img/' + req.file.filename,
        ...
        case 'uploadscrawl':
            ...
            if (req.query.uniqueId) {
                ...
                var dirPath = path.join(dirPathParent, 'img');
                fs.mkdir(dirPathParent, function (err) {
                    if (err && err.code !== 'EEXIST') {
                        next(err);
                    } else {
            ...
            uploadFile(req, res, function (err) {
                if (err) {
                    ...
                    res.json({
                        state: 'SUCCESS',
                        // 此处不能用path.join,因为path会使用'\'分隔符，而url地址必须是'/'分隔符
                        url: '/uploads/' + req.query.uniqueId + '/file/' + req.filename,
                        title: req.file.originalname,
                        original: req.file.originalname,
                        error: null
                    });
